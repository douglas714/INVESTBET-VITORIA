/**
 * Teste de Análise REAL - Com Seleção de Data
...
async function makeRequest(endpoint, description, params = {}) {
  try {
    totalRequests++;
...
// Executar análise
async function runAnalysis(dateType = 'today') {
  try {
    console.log("\n" + "=".repeat(80));
    console.log("🤖 ANÁLISE REAL COM SELEÇÃO DE DATA");
    ...
    console.log(`   Data Selecionada: ${dateLabel} (${dateStr})`);
    ...
    console.log("\n" + "-".repeat(80));
    console.log(`FASE 2: Buscar Fixtures para ${dateLabel}`);
    console.log("-".repeat(80));

    const fixtures = await makeRequest(
      "/fixtures",
      `Buscar fixtures ${dateStr}`,
      { date: dateStr, timezone: "America/Sao_Paulo" }
    );

    let allFixtures = [];
    if (fixtures?.response) allFixtures = fixtures.response;

    console.log(`\n✓ Total de fixtures: ${allFixtures.length}`);

    if (allFixtures.length === 0) {
      console.log(`\n⚠️  Nenhum fixture encontrado para ${dateLabel}`);
    ...
    for (let i = 0; i < Math.min(10, allFixtures.length); i++) {
      ...
      const odds = await makeRequest(
        ...
        { fixture: fixture.id, bookmaker: 8 }
      );
...
// Executar
console.log("\n🚀 TESTE DE ANÁLISE COM SELEÇÃO DE DATA");
console.log("Disparando requisições REAIS...\n");
